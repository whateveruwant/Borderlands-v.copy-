...
void cTransform::CorrectionUp(const D3DXVECTOR3 & _up, float angle)
{
	//진짜로월드 축
	D3DXVECTOR3 worldAxis[3];
	this->GetUnitAxies(worldAxis);
	...
	float distRadian = acos(
		ClampMinusOnePlusOne(D3DXVec3Dot(worldAxis + AXIS_Y, &_up)));
	...
	D3DXVECTOR3 cross;
	D3DXVec3Cross(&cross, worldAxis + AXIS_Y, &_up);
	...
	D3DXVec3Cross(&axis[0], &axis[1], &axis[2]);
...
D3DXVECTOR3 * cTransform::GetWorldpPosition()
{
	D3DXVECTOR3* pos = &(this->position);
