...
void		cSkinnedAnimation::Update(float timeDelta)
{
	...
	if (m_AnimationPlayFactor >= 0.70f) {
...
void cSkinnedAnimation::PlayOneShotNext(std::string animName, std::string animSaveName, float inCrossFadeTime, float outCrossFadeTime)
{
	m_bPlay = true;
	m_bLoop = false;
	m_bSaveNext = true;
	saveAnimname = animSaveName;
	MAP_ANIMSET::iterator find = this->m_mapAnimSet.find(animName);
	if (find != this->m_mapAnimSet.end()) {
