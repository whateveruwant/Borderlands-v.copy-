...
void cPartcleEmitter::ParticleBurst(int num, float minX, float maxX, float minY, float maxY, float minZ, float maxZ, float minSpeed, float maxSpeed, float maxLife, float minLife)
{
	for (int i = 0; i < num; i++)
	{
		D3DXVECTOR3 randVec(
...
void cPartcleEmitter::StartOneParticleMuzle()
{
	//라이브 타임 랜덤
	float liveTime = RandomFloatRange(
		m_fStartLiveTimeMin, m_fStartLiveTimeMax);
