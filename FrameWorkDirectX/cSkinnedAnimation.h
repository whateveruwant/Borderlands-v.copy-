...
public:
	...
	void PlayOneShotNext(std::string animName, std::string animSaveName, float inCrossFadeTime, float outCrossFadeTime = 0.0f);
	...
	bool Gethold() { return is_Hold; }

	bool GetEnd() {
		if (m_fLeftCrossFadeTime <= 0.0f&&m_bLoop)
			return true;
		else
			return false;
	}
